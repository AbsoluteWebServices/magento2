...
export interface UseCompareListFactoryParams<COMPARE_LIST, PRODUCT> extends FactoryParams {
  ...
  removeItems: (context: Context, params: { currentCompareList: COMPARE_LIST; products: PRODUCT[]; customQuery?: CustomQuery }) => Promise<COMPARE_LIST>;
...
export const useCompareListFactory = <COMPARE_LIST, PRODUCT>(
  ...
  return function useCompareList (id: string = ''): UseCompareList<COMPARE_LIST, PRODUCT> {
    ...
    const isInCompareList = ({ product }) => {
      return _factoryParams.isInCompareList({
        currentCompareList: compareList.value,
        product
      });
    };
