...
export const getCustomerGroupData = (
  ...
  customerData?.group_data && customerData?.group_data.length
...
export const getExpirationDays = (
  ...
  return Math.round(
    Math.abs(
      (Number(new Date(verifiedDate)) - Number(new Date(graduationDate))) /
        oneDay
    )
  );
};

export const getVerifiedDate = (locales = 'en-US'): string =>
  new Date().toLocaleDateString(locales);
