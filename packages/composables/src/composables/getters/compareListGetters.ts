...
export const getProducts = (compareList: CompareList): Product[] => getItems(compareList).map(({ product }) => product);

export const getTotals = (compareList: CompareList): AgnosticTotals => {
  return getItems(compareList).reduce((acc, curr) => ({
    total: acc.total + productGetters.getPrice(curr.product).special,
    subtotal: acc.subtotal + productGetters.getPrice(curr.product).regular,
  }), ({ total: 0, subtotal: 0 }));
};
